...
- **`fetch_tier` makes the process verifiable.** The Agent tool returns
  only a narration — the parent cannot see which tool you fetched with.
  Declare it per opened source. `check_subagent_report.py` flags a
  `verbatim_quote` drawn from a `webfetch`/`websearch` tier, and (with
  `--manifest`) cross-checks the declared tier against the recorded
  snapshot `tool`; a disagreement fails loud.
